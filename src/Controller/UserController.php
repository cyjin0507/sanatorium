...
class UserController
{
    ...
    public function idCheck() {
        ...
        if(!$data) {
            falseAlert("해당 아이디가 존재하지 않습니다.", "");
        }
        view('/user/find/passwordChange', ['data'=>$data]);
    }

    public function pwChange() {
        extract($_POST);
        $data = DB::execute("UPDATE users set password=? where id=?", [$pw, $id]);
        if($data) {
            trueAlert("비밀번호 변경이 완료되었습니다.", '/login');
        }
    }
